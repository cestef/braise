use crate::{SpannedToken, create_lexer};
use braise_core::{BraiseError, FileId, Position, Span};
...
/// lex an interpolated string, tokenizing expressions within ${} blocks
pub fn lex_interpolated_string(
    input: &str,
    source: &str,
    start_offset: usize,
) -> Result<InterpolatedString, BraiseError> {
    ...
    while let Some(ch) = chars.next() {
        if ch == '$' && chars.peek() == Some(&'{') {
            ...
            let expr_tokens =
                tokenize_expression(&expr_content, source, start_offset + expr_start)?;
...
fn tokenize_expression(
    ...
    source: &str,
    start_offset: usize,
) -> Result<Vec<SpannedToken>, BraiseError> {
...
mod tests {
    ...
    fn test_simple_interpolation() {
        ...
        assert!(matches!(
            result.segments[1],
            InterpolationSegment::Expression(_)
        ));
        assert!(matches!(
            result.segments[2],
            InterpolationSegment::String(_)
